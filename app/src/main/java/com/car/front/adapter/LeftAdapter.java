...
public class LeftAdapter extends AutoRVAdapter {
    private List<CommodityInfo> inventories = new ArrayList<>();
    private Context mContext;
    private SparseBooleanArray selected;
    private boolean isSingle = true;
    private int old = 0;

    public LeftAdapter(Context context, List<CommodityInfo> inventories) {
    ...
    public void onBindViewHolder(ViewHolder vh, int position) {
        CommodityInfo inventory = inventories.get(position);
        vh.getTextView(R.id.title_text_tv).setText(inventory.getName() +"");
